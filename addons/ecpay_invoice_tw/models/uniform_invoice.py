...
class UniformInvoice(models.Model):
    ...
    IIS_Award_Flag = fields.Selection(string='中獎旗標', selection=[
                                      ('0', '未中獎'), ('1', '已中獎'), ('X', '有統編之發票')])
    ...
    IIS_Carruer_Type = fields.Selection(string='載具類別', selection=[('1', '綠界科技電子發票載具'), ('2', '消費者自然人憑證'),
                                                                  ('3', '消費者手機條碼')])
    IIS_Category = fields.Selection(string='發票類別', selection=[
                                    ('B2B', '有統編'), ('B2C', '無統編')])
    IIS_Check_Number = fields.Char(string='發票檢查碼')
    IIS_Clearance_Mark = fields.Selection(
        string='通關方式', selection=[('1', '經海關出口'), ('2', '非經海關出口')])
    ...
    IIS_Invalid_Status = fields.Selection(string='發票作廢狀態', selection=[
                                          ('1', '已作廢'), ('0', '未作廢')])
    IIS_IP = fields.Char(string='發票開立IP')
    IIS_Issue_Status = fields.Selection(string='發票開立狀態', selection=[
                                        ('1', '發票開立'), ('0', '發票註銷')])
    IIS_Love_Code = fields.Char(string='捐款單位捐贈碼')
    IIS_Mer_ID = fields.Char(string='合作特店編號')
    IIS_Number = fields.Char(string='發票號碼')
    IIS_Print_Flag = fields.Selection(string='列印旗標', selection=[
                                      ('1', '列印'), ('0', '不列印')])
    ...
    IIS_Tax_Type = fields.Selection(string='課稅別', selection=[(
        '1', '應稅'), ('2', '零稅率'), ('3', '免稅'), ('9', '混合應稅與免稅')])
    IIS_Turnkey_Status = fields.Selection(string='發票上傳後接收狀態', selection=[
                                          ('C', '成功'), ('E', '失敗'), ('G', '處理中')])
    IIS_Type = fields.Selection(string='發票種類', selection=[
                                ('07', '一般稅額計算'), ('08', '特種稅額計算')])
    IIS_Upload_Date = fields.Datetime(string='發票上傳時間')
    IIS_Upload_Status = fields.Selection(string='發票上傳狀態', selection=[
                                         ('1', '已上傳'), ('0', '未上傳')])
    ...
    def get_ecpay_invoice_info(self):
        ...
        self.env['account.invoice'].demo_invoice_init(
            invoice, 'Query/Issue', 'INVOICE_SEARCH')
        ...
        # 設定發票的日期
        invoice_create = datetime.datetime.strptime(
            aReturn_Info['IIS_Create_Date'], '%Y-%m-%d %H:%M:%S')
        ...
        if datetime_int == 11 or datetime_int == 12:
            ...
            self.invoice_month = str(date.year - 1911) + '年0' + str(
                datetime_int-1) + '-' + invoice_create.strftime("%m") + '月'
        elif datetime_int % 2 == 1:
            self.invoice_month = str(
                date.year - 1911) + '年' + invoice_create.strftime("%m") + '-0' + str(datetime_int+1) + '月'
